...
export default function RecipeSpecific({ mode }: { mode: "view" | "edit" }) {
  ...
  (async () => {
    ...
    setIngredientsDraft(
      ...
      unit_type: normalizeUnitType(x.unit_type, x.unit_name),
      is_existing: true,
          }))
    );
    setStepsDraft(
 ...
 useEffect(() => {
  ...
  setIngredientsDraft((prev) =>
    ...
    i === idx
          ? {
              ...
              name: pick.name,
              unit_name: first,
              unit_type: normalizeUnitType(pick.unit_type, first),
              is_existing: true,
      }
          : it
      )
  ...
  >
    ...
    <input
          ...
          style={{ marginTop: 20, marginBottom: 12, width: "100%" }}
    />
      )}
    ...
    <textarea
          ...
          style={{ width: "100%", minHeight: 90, marginBottom: 18 }}
    />
      )}
    ...
    <table className={style.table}>
      ...
      {ingredientsView.map((item, idx) => {
        ...
        <tr key={item.local_id}>
          ...
          {!isEdit ? (
                      <span className={style.ingredientName}>{item.name}</span>
          ) : (
                      <>
            <input
            ...
            onChange={(e) =>
              ...
              prev.map((it, i) =>
                ...
                ? {
                  ...
                  unit_name: DEFAULT_UNIT_MAP[e.target.value as UnitType][0] ?? "",
                                  }
                : it
              )
                          )
